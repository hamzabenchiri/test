...
export default function ReactNativeApp({ onSwitchMode }) {
  const [theme, setTheme] = useState('dark');
  const [currency, setCurrency] = useState('USD');
  const [activeTab, setActiveTab] = useState('overview');
  const [showVoiceBanner, setShowVoiceBanner] = useState(() => {
    try {
      return localStorage.getItem('spense_show_voice_banner_v1') !== 'false';
    } catch (e) {
      return true;
    }
  });
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      {/* React Native Architecture Indicator */}
      <View style={[styles.rnBadgeBar, { backgroundColor: colors.cardSubtle, borderColor: colors.border }]}>
        <Text style={[styles.rnBadgeText, { color: '#D2AF26' }]}>
          ⚡ React Native (JavaScript / JSX) Runtime Active
        </Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {/* Voice AI Hero Banner (Dismissible & Remembered) */}
        ...
        <View style={styles.modalBackdrop}>
          <View style={[styles.voiceModal, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={{ color: colors.textSecondary, fontSize: 13, marginBottom: 14 }}>
              Try typing or speaking: <Text style={{ color: '#D2AF26', fontWeight: '600' }}>"Spent $28 on dinner with Apple Pay"</Text>
            </Text>
...
const styles = StyleSheet.create({
  ...
  settingsModal: { width: '100%', maxWidth: 360, padding: 20, borderRadius: 20, borderWidth: 1 },
  voiceModal: { width: '100%', maxWidth: 360, padding: 20, borderRadius: 20, borderWidth: 1 },
  addModal: { width: '100%', maxWidth: 360, padding: 20, borderRadius: 20, borderWidth: 1 },
